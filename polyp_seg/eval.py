...
def eval_for_testAllInOne(opt, pred, gt):
    ...
    meanPr = np.mean(threshold_Pr)
    meanRec = np.mean(threshold_Rec)
...
def evaluate(opt):
    ...
    for dataset in datasets:
        ...
        column_Rec = np.mean(threshold_Recall, axis=0)
        meanRec = np.mean(column_Rec)
...
if __name__ == "__main__":
    eval_config = {
        ...
        "metrics": ['meanDic', 'meanIoU', 'wFm', 'Sm', 'meanEm', 'mae', 'meanPr', 'meanRec'],
    }
    pred_roots = ['results/Shunted_DRADnet']
